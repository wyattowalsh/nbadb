"""Stdlib-only independent NBA Stats body projection decoder.

The module loads the frozen runtime-contract JSON directly and never imports
the production parser, runtime registry, provenance authority, or frame stack.
"""
...
from typing import TYPE_CHECKING, Any, ClassVar, Final, Literal, Self, TypedDict, cast
...
class IndependentStatsProjectionDecoderError(ValueError):
    """Raised when exact parser bytes cannot prove one frozen projection."""


_RUNTIME_RESOURCE = "nba_api_runtime_contract_v1_11_4.json"
_RUNTIME_RESOURCE_SHA256: Final = "24ed29f72f19de9e9c02ed4e4f8363b0e2b0809ce59886975f2f739cc80665b4"
_RUNTIME_PAYLOAD_SHA256: Final = "7c9b59c475cff6b0b9d3619bdfe9a3849e11619980f6d15a1cafd5f269f61b3b"
_STATS_CONTRACTS_SHA256: Final = "a9838c5464d5410210fc3d5af3d3f75c1a796eb46ff999079749a1a4b77f4309"
_PINNED_PROVIDER_AUTHORITY_SHA256: Final = (
    "e5981c0223c99ecd4a483606781d1433cc23f22b26ec71c41a3b12b9ad827085"
)
_DECODER_CONTRACT_SHA256: Final = "3e2b2e9b5c9d42644e59f4382e286465208bea9639270517a4bb40ccde5d169d"


@dataclass(frozen=True, slots=True)
class _PinnedResultSet:
    result_set_name: str
    expected_columns: tuple[str, ...]


@dataclass(frozen=True, slots=True)
class _PinnedContract:
    endpoint_id: str
    endpoint_slug: str
    parser_kind: str
    response_mode: str
    result_sets: tuple[_PinnedResultSet, ...]
    contract_sha256: str


def _fail(message: str) -> IndependentStatsProjectionDecoderError:
    return IndependentStatsProjectionDecoderError(message)
...
def _resource_canonical_sha256(value: object) -> str:
    try:
        ...
        raise _fail("frozen stats contract is not canonical JSON") from None
    return hashlib.sha256(encoded).hexdigest()


@lru_cache(maxsize=1)
def _pinned_runtime_contracts() -> Mapping[str, _PinnedContract]:
    try:
        raw = Path(__file__).with_name(_RUNTIME_RESOURCE).read_bytes()
    except OSError:
        raise _fail("frozen stats contract resource is unavailable") from None
    if hashlib.sha256(raw).hexdigest() != _RUNTIME_RESOURCE_SHA256:
        raise _fail("frozen stats contract resource digest differs from its pin")
    _preflight_json_lexical(raw)
    try:
        payload = json.loads(
            raw,
    ...
    except IndependentStatsProjectionDecoderError:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError, RecursionError, MemoryError):
        raise _fail("frozen stats contract resource cannot be decoded") from None
    if type(payload) is not dict:
        raise _fail("frozen stats contract resource root is invalid")
    exact = cast("dict[str, object]", payload)
    contracts = exact.get("contracts")
    if (
        exact.get("payload_sha256") != _RUNTIME_PAYLOAD_SHA256
        or exact.get("contracts_sha256") != _STATS_CONTRACTS_SHA256
        or type(contracts) is not dict
        or _resource_canonical_sha256(contracts) != _STATS_CONTRACTS_SHA256
        or len(cast("dict[object, object]", contracts)) != 139
    ):
        raise _fail("frozen stats contract resource authority is invalid")
    result: dict[str, _PinnedContract] = {}
    for endpoint_id, item in sorted(cast("dict[object, object]", contracts).items()):
        if type(endpoint_id) is not str or type(item) is not dict:
            raise _fail("frozen stats endpoint contract has a foreign shape")
        row = cast("dict[str, object]", item)
        endpoint_slug = row.get("endpoint_slug")
        parser_kind = row.get("parser_kind")
        raw_routes = row.get("result_sets")
        if (
            type(endpoint_slug) is not str
            or not endpoint_slug
            or parser_kind not in {"legacy_result_sets", "custom_nested"}
            or type(raw_routes) is not list
            or len(cast("list[object]", raw_routes)) > MAX_RESULT_ROUTES
        ):
            raise _fail("frozen stats endpoint contract identity is invalid")
        routes: list[_PinnedResultSet] = []
        for raw_route in cast("list[object]", raw_routes):
            if type(raw_route) is not dict:
                raise _fail("frozen stats result route has a foreign shape")
            route = cast("dict[str, object]", raw_route)
            name = route.get("result_set_name")
            columns = route.get("expected_columns")
            if (
                type(name) is not str
                or _RESULT_NAME_RE.fullmatch(name) is None
                or type(columns) is not list
                or len(cast("list[object]", columns)) > MAX_RESULT_HEADERS
                or any(type(column) is not str for column in cast("list[object]", columns))
            ):
                raise _fail("frozen stats result route identity is invalid")
            routes.append(_PinnedResultSet(name, tuple(cast("list[str]", columns))))
        result[endpoint_id] = _PinnedContract(
            endpoint_id=endpoint_id,
            endpoint_slug=endpoint_slug,
            parser_kind=cast("str", parser_kind),
            response_mode=("unknown_dynamic_response" if not routes else "declared_result_sets"),
            result_sets=tuple(routes),
            contract_sha256=_resource_canonical_sha256(row),
        )
    return MappingProxyType(result)


def pinned_stats_projection_decoder_contract_identities() -> Mapping[str, str]:
    """Return immutable frozen resource/provider/decoder identities."""

    contracts = _pinned_runtime_contracts()
    if len(contracts) != 139:
        raise _fail("frozen stats contract census is invalid")
    return MappingProxyType(
        {
            "decoder_contract_sha256": _DECODER_CONTRACT_SHA256,
            "provider_authority_sha256": _PINNED_PROVIDER_AUTHORITY_SHA256,
            "runtime_contract_payload_sha256": _RUNTIME_PAYLOAD_SHA256,
            "runtime_contract_resource_sha256": _RUNTIME_RESOURCE_SHA256,
            "stats_contracts_sha256": _STATS_CONTRACTS_SHA256,
        }
    )
...
def _is_exact_pinned_zero_header_route(
    *,
    ...
    matches = 0
    for contract in _pinned_runtime_contracts().values():
...
def _canonical_json_bytes(value: object, *, maximum: int) -> bytes:
    try:
        ...
        raise _fail("decoded stats values are not bounded canonical JSON") from None
...
def _observed_typed_frame_is_representable(
    ...
    *,
    ...
    if not rows:
        return True
    # The pure boundary treats exact rectangular JSON with per-column stable
    # non-null kinds as representable.  Structural drift is already sealed by
    # the anomaly inventory and requires the lossless representation.
    for column in range(len(expected_headers)):
        kinds = {_json_value_kind(row[column]) for row in rows if row[column] is not None}
        if len(kinds) > 1:
            return False
    return True
...
def _bounded_json_integer(token: str) -> int:
    ...
    try:
        value = int(token)
    except ValueError:  # pragma: no cover - json validates numeric grammar first
        raise _fail("stats parser input contains an invalid integer token") from None
...
def _bounded_json_float(token: str) -> float:
    ...
    try:
        ...
        raise _fail("stats parser input contains an invalid number token") from None
...
def _preflight_json_lexical(parser_input: bytes) -> None:
    """Bound lexical nesting before the recursive stdlib decoder sees input."""

    try:
        text = parser_input.decode("utf-8", errors="strict")
    except UnicodeDecodeError:
        raise _fail("stats parser input is not exact UTF-8 JSON") from None
    depth = 0
    nodes = 0
    in_string = False
    escaped = False
    ...
    for character in text:
        if in_string:
            ...
            if character == '"':
                in_string = False
                continue
            string_bytes += len(character.encode("utf-8", errors="strict"))
            if string_bytes > MAX_JSON_TOTAL_STRING_BYTES:
                raise _fail("stats parser input cumulative string budget is exhausted")
            continue
        if character == '"':
            ...
            depth -= 1
            if depth < 0:
                raise _fail("stats parser input has invalid lexical structure")
        elif ord(character) < 0x20 and character not in " \t\r\n":
            raise _fail("stats parser input contains an unescaped control character")
        if nodes > MAX_JSON_NODES:
            raise _fail("stats parser input JSON node budget is exhausted")
...
def _decode_json_bytes(parser_input: bytes, *, require_object: bool) -> object:
    ...
    _preflight_json_lexical(parser_input)
    try:
        value = json.loads(
            parser_input,
            object_pairs_hook=_reject_duplicate_object_keys,
        ...
        raise _fail("stats parser input cannot be decoded as exact JSON") from None
...
def _headers_by_name(contract: _PinnedContract) -> dict[str, tuple[str, ...]]:
...
def _decode_observed_unknown_results(
    *,
    payload: dict[str, object],
    budget: _OutputBudget,
) -> tuple[DecodedObservedStatsResultV1, ...]:
    """Decode an exact unknown-mode body without inventing result occurrences.

    The pinned four Video endpoints declare no result-set inventory.  A body
    containing exactly one legacy ``resultSets``/``resultSet`` envelope keeps
    the existing strict occurrence decoder.  Any nonlegacy JSON object has no
    provider result occurrence: its complete value authority is retained by
    the response residual-node projection built later in this module.
    """

    roots = tuple(name for name in ("resultSets", "resultSet") if name in payload)
    if len(roots) > 1:
        raise _fail("observed unknown-dynamic response has ambiguous legacy envelopes")
    if roots:
        return _decode_observed_unknown_legacy_results(
            payload=payload,
            budget=budget,
        )
    return ()
...
def _assert_custom_census(contracts: Mapping[str, _PinnedContract]) -> None:
    custom = {
...
def _admitted_contract(
    *,
    ...
    expected_provider = _PINNED_PROVIDER_AUTHORITY_SHA256
    if (
...
def decode_observed_lossless_stats_response(
    *,
    ...
    if response_mode == "unknown_dynamic_response":
        ...
        results = _decode_observed_unknown_results(
            payload=payload,
            budget=budget,
        )
    ...
    return DecodedObservedStatsResponseV1._build(
        endpoint_id=endpoint_id,
        endpoint_slug=endpoint_slug,
        response_mode=cast("ObservedStatsResponseMode", response_mode),
        expected_headers_by_name={name: headers for name, headers in expected_routes},
        results=results,
    )
...
def _endpoint_contract_sha256_for(contract: _PinnedContract) -> str:
...
_RECORD_KINDS: Final = frozenset(
    {"result_set", "missing_expected", "raw_headers", "raw_rows", "header", "row", "cell"}
)
_HEADER_REFERENCE_KINDS: Final = frozenset({"named", "non_string", "out_of_range"})
_VALUE_KINDS: Final = frozenset(
    {"null", "boolean", "integer", "number", "string", "array", "object"}
)
_PRESENCE_KINDS: Final = frozenset({"present", "null", "empty_object", "empty_array"})
_RESULT_PRESENCE_KINDS: Final = frozenset({"present", "present_empty", "missing"})
_RESPONSE_MODES: Final = frozenset({"declared_result_sets", "unknown_dynamic_response"})
_MAX_PROJECTION_BYTES: Final = 256 * 1024 * 1024
_MAX_PATH_BYTES: Final = 4096


def _projection_sha256(value: object) -> str:
    return hashlib.sha256(_canonical_json_bytes(value, maximum=_MAX_PROJECTION_BYTES)).hexdigest()


def _ordered_projection_root(kind: str, values: Sequence[str]) -> str:
    if type(kind) is not str or not kind or type(values) not in {tuple, list}:
        raise _fail("stats projection root input has a foreign type")
    accumulator = hashlib.sha256()
    accumulator.update(kind.encode("ascii"))
    accumulator.update(b"\0")
    accumulator.update(len(values).to_bytes(8, "big"))
    for value in values:
        if type(value) is not str or _SHA256_RE.fullmatch(value) is None:
            raise _fail("stats projection root contains a foreign digest")
        accumulator.update(bytes.fromhex(value))
...
def _projection_value_kind(value: object) -> str:
    ...
    if type(value) is dict:
        return "object"
    raise _fail("stats projection contains a foreign JSON value")
...
def _strict_projection_row(value: object, *, cls: type[object], label: str) -> dict[str, object]:
    if type(value) is not dict:
        raise _fail(f"{label} is not one exact object")
    ...
    if any(type(key) is not str for key in row) or set(row) != expected:
        raise _fail(f"{label} has foreign or missing fields")
    expected_kind = getattr(cls, "kind", None)
    if (
        type(row["schema_version"]) is not int
        or row["schema_version"] != _PROJECTION_SCHEMA_VERSION
    ):
        raise _fail(f"{label} has a foreign schema version")
    if type(row["kind"]) is not str or row["kind"] != expected_kind:
        raise _fail(f"{label} has a foreign kind")
    return cast("dict[str, object]", row)


def _decode_projection_bytes(value: object, *, label: str) -> dict[str, object]:
    if type(value) is not bytes or not value or len(value) > _MAX_PROJECTION_BYTES:
        raise _fail(f"{label} canonical bytes are absent or over bound")
    ...
    try:
        ...
        raise _fail(f"{label} canonical bytes cannot be decoded") from None
    if (
        type(decoded) is not dict
        or _canonical_json_bytes(decoded, maximum=_MAX_PROJECTION_BYTES) != raw
    ):
        raise _fail(f"{label} bytes are not exact canonical JSON")
    return cast("dict[str, object]", decoded)


def _canonical_value_fields(value: object) -> tuple[str, str, str, str]:
    canonical = _canonical_json_bytes(value, maximum=MAX_RESULT_CANONICAL_BYTES).decode("utf-8")
    return (
        _projection_presence_kind(value),
        _projection_value_kind(value),
        canonical,
        hashlib.sha256(canonical.encode("utf-8")).hexdigest(),
    )


def _exact_utf8_bytes(value: object, *, label: str) -> bytes:
    if type(value) is not str:
        raise _fail(f"{label} is not exact text")
    try:
        return value.encode("utf-8", errors="strict")
    except UnicodeEncodeError:
        raise _fail(f"{label} is not exact UTF-8 text") from None


@dataclass(frozen=True, slots=True)
class DecodedStatsProjectionRecordV1:
    record_sha256: str
    record_ordinal: int
    result_record_ordinal: int
    record_kind: str
    result_name: str
    ...
    header_reference_kind: str | None
    header_name: str | None
    header_ordinal: int | None
    header_value_sha256: str | None
    header_duplicate_ordinal: int | None
    row_ordinal: int | None
    row_value_sha256: str | None
    row_duplicate_ordinal: int | None
    cell_ordinal: int | None
    value_duplicate_ordinal: int | None
    presence_kind: str
    value_kind: str
    canonical_json: str
    canonical_json_sha256: str

    schema_version: ClassVar[int] = _PROJECTION_SCHEMA_VERSION
    kind: ClassVar[str] = "decoded_stats_projection_record_v1"

    def __post_init__(self) -> None:
        if type(self.record_sha256) is not str or _SHA256_RE.fullmatch(self.record_sha256) is None:
            raise _fail("stats projection record digest is invalid")
        for value, label, maximum in (
            (self.record_ordinal, "record ordinal", MAX_TOTAL_OUTPUT_CELLS + MAX_RESULT_ROWS),
            (
                self.result_record_ordinal,
                "result record ordinal",
                MAX_TOTAL_OUTPUT_CELLS + MAX_RESULT_ROWS,
            ),
            (self.result_duplicate_ordinal, "result duplicate ordinal", MAX_RESULT_ROUTES - 1),
        ):
            if type(value) is not int or value < 0 or value > maximum:
                raise _fail(f"stats projection {label} is invalid")
        for value, label, maximum in (
            (self.provider_result_ordinal, "provider result ordinal", MAX_RESULT_ROUTES - 1),
            (self.expected_result_ordinal, "expected result ordinal", MAX_RESULT_ROUTES - 1),
            (self.canonical_result_ordinal, "canonical result ordinal", MAX_RESULT_ROUTES - 1),
            (self.header_ordinal, "header ordinal", MAX_RESULT_HEADERS - 1),
            (self.header_duplicate_ordinal, "header duplicate ordinal", MAX_RESULT_HEADERS - 1),
            (self.row_ordinal, "row ordinal", MAX_RESULT_ROWS - 1),
            (self.row_duplicate_ordinal, "row duplicate ordinal", MAX_RESULT_ROWS - 1),
            (self.cell_ordinal, "cell ordinal", MAX_TOTAL_OUTPUT_CELLS - 1),
            (self.value_duplicate_ordinal, "value duplicate ordinal", MAX_TOTAL_OUTPUT_CELLS - 1),
        ):
        ...
        if type(self.record_kind) is not str or self.record_kind not in _RECORD_KINDS:
            raise _fail("stats projection record kind is invalid")
        if type(self.result_name) is not str or _RESULT_NAME_RE.fullmatch(self.result_name) is None:
            raise _fail("stats projection record result name is invalid")
        for digest, label in (
            (self.header_value_sha256, "header value digest"),
            (self.row_value_sha256, "row value digest"),
            (self.canonical_json_sha256, "canonical value digest"),
        ):
            if digest is not None and (
                type(digest) is not str or _SHA256_RE.fullmatch(digest) is None
            ):
                raise _fail(f"stats projection {label} is invalid")
        if self.header_reference_kind is not None and (
            type(self.header_reference_kind) is not str
            or self.header_reference_kind not in _HEADER_REFERENCE_KINDS
        ):
            raise _fail("stats projection header reference kind is invalid")
        header_shape = (
            self.header_reference_kind,
            self.header_name,
            self.header_ordinal,
            self.header_value_sha256,
            self.header_duplicate_ordinal,
        )
        if self.record_kind in {"header", "cell"}:
            if self.header_reference_kind is None or self.header_ordinal is None:
                raise _fail("stats projection header-owned record lacks its slot")
            if self.header_reference_kind == "named":
                if (
                    type(self.header_name) is not str
                    or self.header_value_sha256 is None
                    or self.header_duplicate_ordinal is None
                ):
                    raise _fail("stats projection named header slot is incomplete")
            elif self.header_reference_kind == "non_string":
                if (
                    self.header_name is not None
                    or self.header_value_sha256 is None
                    or self.header_duplicate_ordinal is None
                ):
                    raise _fail("stats projection non-string header slot is incomplete")
            elif any(
                value is not None
                for value in (
                    self.header_name,
                ...
                raise _fail("stats projection out-of-range header slot fabricates a value")
        elif any(value is not None for value in header_shape):
            raise _fail("stats projection non-header record carries header coordinates")
        if self.record_kind in {"row", "cell"}:
            if (
                self.row_ordinal is None
                or self.row_value_sha256 is None
                or self.row_duplicate_ordinal is None
            ):
                raise _fail("stats projection row-owned record lacks its exact row identity")
        elif any(
            value is not None
            for value in (self.row_ordinal, self.row_value_sha256, self.row_duplicate_ordinal)
        ):
            raise _fail("stats projection non-row record carries row coordinates")
        if self.record_kind == "cell":
            if self.cell_ordinal is None or self.value_duplicate_ordinal is None:
                raise _fail("stats projection cell lacks its exact cell identity")
        elif self.cell_ordinal is not None or self.value_duplicate_ordinal is not None:
            raise _fail("stats projection non-cell record carries cell coordinates")
        if (
            ...
            raise _fail("stats projection record value tags are invalid")
        canonical_bytes = _exact_utf8_bytes(
            self.canonical_json,
            label="stats projection canonical value",
        )
        value = _decode_json_bytes(canonical_bytes, require_object=False)
        if (
            self.canonical_json
            != _canonical_json_bytes(value, maximum=MAX_RESULT_CANONICAL_BYTES).decode("utf-8")
            or self.canonical_json_sha256 != hashlib.sha256(canonical_bytes).hexdigest()
            or self.presence_kind != _projection_presence_kind(value)
            or self.value_kind != _projection_value_kind(value)
        ):
            raise _fail("stats projection canonical value tags or digest are invalid")
        if self.record_sha256 != _projection_sha256(self.identity_payload()):
            raise _fail("stats projection record digest differs from its identity")

    def identity_payload(self) -> dict[str, object]:
        return {
            "schema_version": self.schema_version,
            "kind": self.kind,
            **{f.name: getattr(self, f.name) for f in fields(self) if f.name != "record_sha256"},
        }
    ...
    def from_row(cls, value: object) -> Self:
        row = _strict_projection_row(value, cls=cls, label="stats projection record")
        try:
            return cls(**cast("Any", {field.name: row[field.name] for field in fields(cls)}))
        except IndependentStatsProjectionDecoderError:
            raise
        except (TypeError, ValueError, RecursionError):
            raise _fail("stats projection record reconstruction failed") from None


@dataclass(frozen=True, slots=True)
class DecodedStatsProjectionResultV1:
    result_sha256: str
    result_ordinal: int
    result_name: str
    result_duplicate_ordinal: int
    ...
    canonical_result_ordinal: int | None
    presence: str
    ordered_header_slots_json: str
    ordered_header_slots_sha256: str
    ...
    records: tuple[DecodedStatsProjectionRecordV1, ...]

    schema_version: ClassVar[int] = _PROJECTION_SCHEMA_VERSION
    kind: ClassVar[str] = "decoded_stats_projection_result_v1"

    def __post_init__(self) -> None:
        for digest in (
            self.result_sha256,
            self.ordered_header_slots_sha256,
            self.anomaly_codes_sha256,
            self.normalized_output_sha256,
            self.raw_headers_sha256,
            self.raw_rows_sha256,
            self.record_root_sha256,
        ):
            if type(digest) is not str or _SHA256_RE.fullmatch(digest) is None:
                raise _fail("stats projection result contains an invalid digest")
        for value, maximum in (
            (self.result_ordinal, MAX_RESULT_ROUTES - 1),
            (self.result_duplicate_ordinal, MAX_RESULT_ROUTES - 1),
            (self.header_record_count, MAX_RESULT_HEADERS),
            (self.header_slot_count, MAX_RESULT_HEADERS),
            (self.raw_row_occurrence_count, MAX_RESULT_ROWS),
            (self.sequence_row_count, MAX_RESULT_ROWS),
            (self.cell_count, MAX_TOTAL_OUTPUT_CELLS),
            (self.record_count, MAX_TOTAL_OUTPUT_CELLS + MAX_RESULT_ROWS + MAX_RESULT_HEADERS + 3),
        ):
            if type(value) is not int or value < 0 or value > maximum:
                raise _fail("stats projection result count or ordinal is invalid")
        for value, label in (
            (self.provider_result_ordinal, "provider result ordinal"),
            (self.expected_result_ordinal, "expected result ordinal"),
            (self.canonical_result_ordinal, "canonical result ordinal"),
        ):
            if value is not None and (
                type(value) is not int or value < 0 or value >= MAX_RESULT_ROUTES
            ):
                raise _fail(f"stats projection result {label} is invalid")
        if type(self.result_name) is not str or _RESULT_NAME_RE.fullmatch(self.result_name) is None:
            raise _fail("stats projection result name is invalid")
        if type(self.presence) is not str or self.presence not in _RESULT_PRESENCE_KINDS:
            raise _fail("stats projection result presence is invalid")
        if (
            type(self.records) is not tuple
            or len(self.records) != self.record_count
            or any(type(item) is not DecodedStatsProjectionRecordV1 for item in self.records)
        ):
            raise _fail("stats projection result record inventory is invalid")
        rebuilt = tuple(
            DecodedStatsProjectionRecordV1.from_row(item.to_row()) for item in self.records
        )
        object.__setattr__(self, "records", rebuilt)
        if tuple(item.result_record_ordinal for item in rebuilt) != tuple(range(len(rebuilt))):
            raise _fail("stats projection result record ordinals are not contiguous")
        if any(
            item.result_name != self.result_name
            or item.result_duplicate_ordinal != self.result_duplicate_ordinal
            or item.provider_result_ordinal != self.provider_result_ordinal
            or item.expected_result_ordinal != self.expected_result_ordinal
            or item.canonical_result_ordinal != self.canonical_result_ordinal
            for item in rebuilt
        ):
            raise _fail("stats projection result record owner is inconsistent")
        if self.record_root_sha256 != _ordered_projection_root(
            "decoded_stats_projection_result_records_v1",
            tuple(item.record_sha256 for item in rebuilt),
        ):
            raise _fail("stats projection result record root is invalid")
        for text, digest, label in (
            (self.ordered_header_slots_json, self.ordered_header_slots_sha256, "header slots"),
            (self.anomaly_codes_json, self.anomaly_codes_sha256, "anomaly codes"),
            (self.raw_headers_json, self.raw_headers_sha256, "raw headers"),
            (self.raw_rows_json, self.raw_rows_sha256, "raw rows"),
        ):
            encoded = _exact_utf8_bytes(text, label=f"stats projection result {label}")
            if digest != hashlib.sha256(encoded).hexdigest():
                raise _fail(f"stats projection result {label} digest is invalid")
            decoded = _decode_json_bytes(encoded, require_object=False)
            if text != _canonical_json_bytes(decoded, maximum=MAX_RESULT_CANONICAL_BYTES).decode(
                "utf-8"
            ):
                raise _fail(f"stats projection result {label} are not canonical")
        slots = _decode_json_bytes(
            _exact_utf8_bytes(
                self.ordered_header_slots_json,
                label="stats projection result header slots",
            ),
            require_object=False,
        )
        if type(slots) is not list or len(cast("list[object]", slots)) != self.header_slot_count:
            raise _fail("stats projection result header-slot denominator is invalid")
        if self.result_sha256 != _projection_sha256(self.identity_payload()):
            raise _fail("stats projection result digest differs from its identity")
    ...
    def identity_payload(self) -> dict[str, object]:
        return {
            ...
            **{
                f.name: (
                    [item.to_row() for item in self.records]
                    if f.name == "records"
                    else getattr(self, f.name)
                )
                for f in fields(self)
                if f.name != "result_sha256"
            },
        }
    ...
    @classmethod
    def from_row(cls, value: object) -> Self:
        row = _strict_projection_row(value, cls=cls, label="stats projection result")
        records = row["records"]
        if type(records) is not list:
            raise _fail("stats projection result records are not one exact array")
        values = {field.name: row[field.name] for field in fields(cls)}
        values["records"] = tuple(
            DecodedStatsProjectionRecordV1.from_row(item) for item in cast("list[object]", records)
        )
        try:
            ...
            raise _fail("stats projection result reconstruction failed") from None


@dataclass(frozen=True, slots=True)
class DecodedStatsResidualNodeV1:
    node_sha256: str
    node_ordinal: int
    ...
    schema_version: ClassVar[int] = _PROJECTION_SCHEMA_VERSION
    kind: ClassVar[str] = "decoded_stats_residual_node_v1"

    def __post_init__(self) -> None:
        if type(self.node_sha256) is not str or _SHA256_RE.fullmatch(self.node_sha256) is None:
            raise _fail("stats residual node digest is invalid")
        if (
            type(self.node_ordinal) is not int
            or self.node_ordinal < 0
            or self.node_ordinal >= MAX_JSON_NODES
        ):
            raise _fail("stats residual node ordinal is invalid")
        if type(self.depth) is not int or self.depth < 0 or self.depth > MAX_JSON_DEPTH:
            raise _fail("stats residual node depth is invalid")
        json_path_bytes = _exact_utf8_bytes(self.json_path, label="stats residual node path")
        if not self.json_path or len(json_path_bytes) > _MAX_PATH_BYTES:
            raise _fail("stats residual node path is invalid")
        if self.parent_json_path is not None:
            parent_path_bytes = _exact_utf8_bytes(
                self.parent_json_path,
                label="stats residual parent path",
            )
            if not self.parent_json_path or len(parent_path_bytes) > _MAX_PATH_BYTES:
                raise _fail("stats residual parent path is invalid")
        if self.object_key is not None:
            object_key_bytes = _exact_utf8_bytes(
                self.object_key,
                label="stats residual object key",
            )
            if len(object_key_bytes) > MAX_JSON_STRING_BYTES:
                raise _fail("stats residual object key exceeds its bound")
        for value, label in (
            (self.object_key_ordinal, "object-key ordinal"),
            (self.array_ordinal, "array ordinal"),
        ):
            if value is not None and (
                type(value) is not int or value < 0 or value >= MAX_JSON_CONTAINER_ITEMS
            ):
                raise _fail(f"stats residual {label} is invalid")
        if self.parent_node_ordinal is None:
            if (
                ...
                or any(
                    value is not None
                    for value in (self.object_key, self.object_key_ordinal, self.array_ordinal)
                )
            ):
                raise _fail("stats residual root node is invalid")
        elif (
            type(self.parent_node_ordinal) is not int
            or self.parent_node_ordinal < 0
            or self.parent_node_ordinal >= self.node_ordinal
            or type(self.parent_json_path) is not str
            or self.depth < 1
            or ((self.object_key is not None) != (self.object_key_ordinal is not None))
            or ((self.object_key is None) == (self.array_ordinal is None))
        ):
            raise _fail("stats residual child node is invalid")
        if (
            type(self.presence_kind) is not str
            or self.presence_kind not in _PRESENCE_KINDS
            or type(self.value_kind) is not str
            ...
            raise _fail("stats residual node value tags are invalid")
        structural = self.presence_kind == "present" and self.value_kind in {"array", "object"}
        if structural:
            if self.canonical_json is not None or self.canonical_json_sha256 is not None:
                raise _fail("stats residual structural node duplicates its subtree")
        else:
            if type(self.canonical_json_sha256) is not str:
                raise _fail("stats residual materialized node lacks its canonical value")
            canonical_bytes = _exact_utf8_bytes(
                self.canonical_json,
                label="stats residual canonical value",
            )
            value = _decode_json_bytes(canonical_bytes, require_object=False)
            if (
                self.canonical_json
                != _canonical_json_bytes(value, maximum=MAX_RESULT_CANONICAL_BYTES).decode()
                or self.canonical_json_sha256 != hashlib.sha256(canonical_bytes).hexdigest()
                or self.presence_kind != _projection_presence_kind(value)
                or self.value_kind != _projection_value_kind(value)
            ):
                raise _fail("stats residual node canonical value is invalid")
        if self.node_sha256 != _projection_sha256(self.identity_payload()):
            raise _fail("stats residual node digest differs from its identity")

    def identity_payload(self) -> dict[str, object]:
        return {
            "schema_version": self.schema_version,
            "kind": self.kind,
            **{f.name: getattr(self, f.name) for f in fields(self) if f.name != "node_sha256"},
        }
    ...
    @classmethod
    def from_row(cls, value: object) -> Self:
        row = _strict_projection_row(value, cls=cls, label="stats residual node")
        try:
            return cls(**cast("Any", {field.name: row[field.name] for field in fields(cls)}))
        except IndependentStatsProjectionDecoderError:
            raise
        except (TypeError, ValueError, RecursionError):
            raise _fail("stats residual node reconstruction failed") from None


@dataclass(frozen=True, slots=True)
class DecodedStatsProjectionResponseV1:
    response_sha256: str
    endpoint_id: str
    endpoint_slug: str
    endpoint_contract_sha256: str
    ...
    results: tuple[DecodedStatsProjectionResultV1, ...]
    residual_node_count: int
    residual_node_root_sha256: str
    residual_nodes: tuple[DecodedStatsResidualNodeV1, ...]
    residual_record_count: int
    residual_record_root_sha256: str
    canonical_payload_sha256: str

    schema_version: ClassVar[int] = _PROJECTION_SCHEMA_VERSION
    kind: ClassVar[str] = "decoded_stats_projection_response_v1"

    def __post_init__(self) -> None:
        for value, label in (
            (self.endpoint_id, "endpoint id"),
            (self.endpoint_slug, "endpoint slug"),
        ):
            if type(value) is not str or not value or value.strip() != value:
                raise _fail(f"stats projection response {label} is invalid")
        if type(self.response_mode) is not str or self.response_mode not in _RESPONSE_MODES:
            raise _fail("stats projection response mode is invalid")
        for digest in (
            self.response_sha256,
            self.endpoint_contract_sha256,
            self.provider_authority_sha256,
            self.decoder_contract_sha256,
            self.parser_input_sha256,
            self.anomaly_codes_sha256,
            self.result_root_sha256,
            self.residual_node_root_sha256,
            self.residual_record_root_sha256,
            self.canonical_payload_sha256,
        ):
            if type(digest) is not str or _SHA256_RE.fullmatch(digest) is None:
                raise _fail("stats projection response contains an invalid digest")
        if (
            type(self.parser_input_length) is not int
            or self.parser_input_length < 1
            or self.parser_input_length > MAX_PARSER_INPUT_BYTES
        ):
            raise _fail("stats projection response parser-input length is invalid")
        for value, maximum in (
            (self.provider_result_set_count, MAX_RESULT_ROUTES),
            (self.expected_result_set_count, MAX_RESULT_ROUTES),
            (self.result_count, MAX_RESULT_ROUTES),
            (self.residual_node_count, MAX_JSON_NODES),
            (self.residual_record_count, MAX_JSON_NODES + 1),
        ):
            if type(value) is not int or value < 0 or value > maximum:
                raise _fail("stats projection response count or length is invalid")
        if (
            type(self.results) is not tuple
            or len(self.results) != self.result_count
            or any(type(item) is not DecodedStatsProjectionResultV1 for item in self.results)
        ):
            raise _fail("stats projection response result inventory is invalid")
        if (
            type(self.residual_nodes) is not tuple
            or len(self.residual_nodes) != self.residual_node_count
            or any(type(item) is not DecodedStatsResidualNodeV1 for item in self.residual_nodes)
        ):
            raise _fail("stats projection response residual inventory is invalid")
        rebuilt_results = tuple(
            DecodedStatsProjectionResultV1.from_row(item.to_row()) for item in self.results
        )
        rebuilt_nodes = tuple(
            DecodedStatsResidualNodeV1.from_row(item.to_row()) for item in self.residual_nodes
        )
        object.__setattr__(self, "results", rebuilt_results)
        object.__setattr__(self, "residual_nodes", rebuilt_nodes)
        if tuple(item.result_ordinal for item in rebuilt_results) != tuple(
            range(len(rebuilt_results))
        ):
            raise _fail("stats projection response result ordinals are not contiguous")
        if tuple(item.node_ordinal for item in rebuilt_nodes) != tuple(range(len(rebuilt_nodes))):
            raise _fail("stats projection response node ordinals are not contiguous")
        contract = _admitted_contract(
            endpoint_id=self.endpoint_id,
            endpoint_contract_sha256_value=self.endpoint_contract_sha256,
            provider_authority_sha256=self.provider_authority_sha256,
        )
        if (
            self.endpoint_slug != contract.endpoint_slug
            or self.response_mode != contract.response_mode
            or self.expected_result_set_count != len(contract.result_sets)
            or self.decoder_contract_sha256 != _DECODER_CONTRACT_SHA256
        ):
            raise _fail("stats projection response differs from its frozen contract")
        if self.result_root_sha256 != _ordered_projection_root(
            "decoded_stats_projection_results_v1",
            tuple(item.result_sha256 for item in rebuilt_results),
        ):
            raise _fail("stats projection response result root is invalid")
        if self.residual_node_root_sha256 != _ordered_projection_root(
            "decoded_stats_projection_residual_nodes_v1",
            tuple(item.node_sha256 for item in rebuilt_nodes),
        ):
            raise _fail("stats projection response residual-node root is invalid")
        expected_residual_root = _ordered_projection_root(
            "decoded_stats_projection_residual_records_v1",
            (self.canonical_payload_sha256, *(item.node_sha256 for item in rebuilt_nodes)),
        )
        if (
            self.residual_record_count != len(rebuilt_nodes) + 1
            or self.residual_record_root_sha256 != expected_residual_root
        ):
            raise _fail("stats projection response residual-record inventory is invalid")
        if (
            self.anomaly_codes_sha256
            != hashlib.sha256(
                _exact_utf8_bytes(
                    self.anomaly_codes_json,
                    label="stats projection response anomaly inventory",
                )
            ).hexdigest()
        ):
            raise _fail("stats projection response anomaly inventory is invalid")
        anomalies = _decode_json_bytes(
            _exact_utf8_bytes(
                self.anomaly_codes_json,
                label="stats projection response anomaly inventory",
            ),
        ...
        if (
            ...
            or any(type(item) is not str for item in cast("list[object]", anomalies))
            or cast("list[str]", anomalies) != sorted(set(cast("list[str]", anomalies)))
        ):
            raise _fail("stats projection response anomaly inventory is invalid")
        child_counts: Counter[int] = Counter()
        children_by_parent: dict[int, list[DecodedStatsResidualNodeV1]] = {}
        edge_kinds: dict[int, str] = {}
        edge_ordinals: dict[tuple[int, str], int] = {}
        object_keys: dict[int, set[str]] = {}
        for node in rebuilt_nodes:
            parent_ordinal = node.parent_node_ordinal
            if parent_ordinal is None:
                continue
            parent = rebuilt_nodes[parent_ordinal]
            if parent.value_kind not in {"array", "object"} or parent.presence_kind != "present":
                raise _fail("stats residual scalar or empty node owns a child")
            if node.parent_json_path != parent.json_path or node.depth != parent.depth + 1:
                raise _fail("stats residual node parent path or depth is invalid")
            object_edge = node.object_key is not None
            edge_kind = "object" if object_edge else "array"
            if parent.value_kind != edge_kind:
                raise _fail("stats residual node edge contradicts its parent kind")
            if edge_kinds.setdefault(parent_ordinal, edge_kind) != edge_kind:
                raise _fail("stats residual parent mixes child edge kinds")
            edge_ordinal = cast(
                "int",
                node.object_key_ordinal if object_edge else node.array_ordinal,
            )
            edge_key = (parent_ordinal, edge_kind)
            if edge_ordinal != edge_ordinals.get(edge_key, 0):
                raise _fail("stats residual sibling order is not contiguous")
            edge_ordinals[edge_key] = edge_ordinal + 1
            object_path = _canonical_json_bytes(node.object_key, maximum=4096).decode()
            expected_path = (
                f"{parent.json_path}[{object_path}]"
                if object_edge
                else f"{parent.json_path}[{edge_ordinal}]"
            )
            if node.json_path != expected_path:
                raise _fail("stats residual child path is not canonical")
            if object_edge:
                keys = object_keys.setdefault(parent_ordinal, set())
                if node.object_key in keys:
                    raise _fail("stats residual object repeats a child key")
                keys.add(node.object_key)
            child_counts[parent_ordinal] += 1
            children_by_parent.setdefault(parent_ordinal, []).append(node)
        for node in rebuilt_nodes:
            structural = node.presence_kind == "present" and node.value_kind in {
                "array",
                "object",
            }
            if structural != (child_counts[node.node_ordinal] > 0):
                raise _fail("stats residual structural-container proof is inconsistent")
        reconstructed_values: list[object] = [None] * len(rebuilt_nodes)
        for node in reversed(rebuilt_nodes):
            if node.canonical_json is not None:
                reconstructed_values[node.node_ordinal] = _decode_json_bytes(
                    node.canonical_json.encode(), require_object=False
                )
                continue
            children = children_by_parent.get(node.node_ordinal, [])
            if node.value_kind == "object":
                reconstructed_values[node.node_ordinal] = {
                    cast("str", child.object_key): reconstructed_values[child.node_ordinal]
                    for child in children
                }
            elif node.value_kind == "array":
                reconstructed_values[node.node_ordinal] = [
                    reconstructed_values[child.node_ordinal] for child in children
                ]
            else:  # pragma: no cover - structural tags are closed above
                raise _fail("stats residual structural node has a foreign kind")
        if not reconstructed_values or type(reconstructed_values[0]) is not dict:
            raise _fail("stats residual inventory does not reconstruct one response object")
        reconstructed_payload = cast("dict[str, object]", reconstructed_values[0])
        canonical_nodes = _build_residual_nodes(reconstructed_payload)
        if rebuilt_nodes != canonical_nodes:
            raise _fail("stats residual node inventory is not canonical preorder")
        if self.canonical_payload_sha256 != _projection_sha256(reconstructed_payload):
            raise _fail("stats residual inventory differs from its canonical payload root")
        try:
            ...
            raise _fail("stats residual inventory cannot be replayed") from None
        if len(reconstructed_bytes) > MAX_PARSER_INPUT_BYTES:
            raise _fail("stats residual reconstructed response exceeds its bound")
        observed = decode_observed_lossless_stats_response(
            endpoint_id=self.endpoint_id,
            endpoint_contract_sha256=self.endpoint_contract_sha256,
            provider_authority_sha256=self.provider_authority_sha256,
            parser_input=reconstructed_bytes,
        )
        expected_results: list[DecodedStatsProjectionResultV1] = []
        global_record_ordinal = 0
        for result_ordinal, observed_result in enumerate(observed.results):
            expected_result, global_record_ordinal = _build_projection_result(
                result_ordinal=result_ordinal,
                result=observed_result,
                contract=contract,
                global_record_start=global_record_ordinal,
            )
            expected_results.append(expected_result)
        if tuple(expected_results) != rebuilt_results:
            raise _fail("stats projection results differ from the residual response")
        if (
            self.provider_result_set_count != observed.provider_result_set_count
            or self.expected_result_set_count != observed.expected_result_set_count
            or self.response_mode != observed.response_mode
        ):
            raise _fail("stats projection response denominators differ from residual replay")
        if tuple(cast("list[str]", anomalies)) != observed.reason_codes:
            raise _fail("stats projection response anomalies differ from residual replay")
        if self.response_sha256 != _projection_sha256(self.identity_payload()):
            raise _fail("stats projection response digest differs from its identity")
    ...
    def identity_payload(self) -> dict[str, object]:
        return {
            ...
            **{
                f.name: (
                    [item.to_row() for item in self.results]
                    if f.name == "results"
                    else [item.to_row() for item in self.residual_nodes]
                    if f.name == "residual_nodes"
                    else getattr(self, f.name)
                )
                for f in fields(self)
                if f.name != "response_sha256"
            },
        }
    ...
    def canonical_bytes(self) -> bytes:
        return _canonical_json_bytes(self.to_row(), maximum=_MAX_PROJECTION_BYTES)

    @classmethod
    def from_row(cls, value: object) -> Self:
        row = _strict_projection_row(value, cls=cls, label="stats projection response")
        raw_results = row["results"]
        raw_nodes = row["residual_nodes"]
        if type(raw_results) is not list or type(raw_nodes) is not list:
            raise _fail("stats projection response nested inventories are invalid")
        values = {field.name: row[field.name] for field in fields(cls)}
        values["results"] = tuple(
            DecodedStatsProjectionResultV1.from_row(item)
            for item in cast("list[object]", raw_results)
        )
        values["residual_nodes"] = tuple(
            DecodedStatsResidualNodeV1.from_row(item) for item in cast("list[object]", raw_nodes)
        )
        try:
            return cls(**cast("Any", values))
        except IndependentStatsProjectionDecoderError:
            ...
            raise _fail("stats projection response reconstruction failed") from None

    @classmethod
    def from_canonical_bytes(cls, value: object) -> Self:
        row = _decode_projection_bytes(value, label="stats projection response")
        candidate = cls.from_row(row)
        if candidate.canonical_bytes() != value:
            raise _fail("stats projection response canonical replay drifted")
        return candidate


class _ProjectionRecordMetadata(TypedDict, total=False):
    header_reference_kind: str | None
    header_name: str | None
    header_ordinal: int | None
    header_value_sha256: str | None
...
class _ProjectionHeaderSlot(TypedDict):
    header_reference_kind: str
    header_name: str | None
    header_value_sha256: str | None
...
def _build_projection_record(
    *,
    ...
    result: DecodedObservedStatsResultV1,
    expected_result_ordinal: int | None,
    header_reference_kind: str | None = None,
    header_name: str | None = None,
    header_ordinal: int | None = None,
    header_value_sha256: str | None = None,
    header_duplicate_ordinal: int | None = None,
    row_ordinal: int | None = None,
    row_value_sha256: str | None = None,
    row_duplicate_ordinal: int | None = None,
    cell_ordinal: int | None = None,
    value_duplicate_ordinal: int | None = None,
    value: object,
) -> DecodedStatsProjectionRecordV1:
    presence, value_kind, canonical, canonical_sha = _canonical_value_fields(value)
    values: dict[str, object] = {
        "record_ordinal": record_ordinal,
        "result_record_ordinal": result_record_ordinal,
        "record_kind": record_kind,
        "result_name": result.result_name,
        "result_duplicate_ordinal": result.duplicate_name_ordinal,
        "provider_result_ordinal": result.provider_ordinal,
        "expected_result_ordinal": expected_result_ordinal,
        "canonical_result_ordinal": result.canonical_ordinal,
        "header_reference_kind": header_reference_kind,
        "header_name": header_name,
        "header_ordinal": header_ordinal,
        "header_value_sha256": header_value_sha256,
        "header_duplicate_ordinal": header_duplicate_ordinal,
        "row_ordinal": row_ordinal,
        "row_value_sha256": row_value_sha256,
        "row_duplicate_ordinal": row_duplicate_ordinal,
        "cell_ordinal": cell_ordinal,
        "value_duplicate_ordinal": value_duplicate_ordinal,
    ...
    payload = {
        ...
        "kind": DecodedStatsProjectionRecordV1.kind,
        **values,
    }
    return DecodedStatsProjectionRecordV1(
        record_sha256=_projection_sha256(payload), **cast("Any", values)
    )
...
def _build_projection_result(
    *,
...
) -> tuple[DecodedStatsProjectionResultV1, int]:
    expected_by_name = {
        route.result_set_name: ordinal for ordinal, route in enumerate(contract.result_sets)
    }
    expected_headers_by_name = {
        route.result_set_name: route.expected_columns for route in contract.result_sets
    }
    expected_ordinal = expected_by_name.get(result.result_name)
    expected_headers = expected_headers_by_name.get(result.result_name)
    raw_headers = result.raw_header_container
    raw_rows = result.raw_row_container
    ...
    maximum_width = max(
        (len(cast("list[object]", row)) for row in row_values if type(row) is list), default=0
    )
    header_slot_count = max(len(header_values), maximum_width)
    header_value_digests = [
        hashlib.sha256(_canonical_json_bytes(value, maximum=MAX_RESULT_CANONICAL_BYTES)).hexdigest()
        for value in header_values
    ]
    duplicate_counts: Counter[str] = Counter()
    header_duplicates: list[int] = []
    slots: list[_ProjectionHeaderSlot] = []
    for ordinal in range(header_slot_count):
        if ordinal >= len(header_values):
            slots.append(
                {
            ...
            continue
        value = header_values[ordinal]
        digest = header_value_digests[ordinal]
        header_duplicates.append(duplicate_counts[digest])
        duplicate_counts[digest] += 1
        slots.append(
            {
                "header_reference_kind": "named" if type(value) is str else "non_string",
                "header_name": value if type(value) is str else None,
                "header_value_sha256": digest,
            }
        )
    slot_json = _canonical_json_bytes(slots, maximum=MAX_RESULT_CANONICAL_BYTES).decode()
    row_digests = [
        hashlib.sha256(_canonical_json_bytes(value, maximum=MAX_RESULT_CANONICAL_BYTES)).hexdigest()
        for value in row_values
    ]
    row_counts: Counter[str] = Counter()
    row_duplicates: list[int] = []
    for digest in row_digests:
        row_duplicates.append(row_counts[digest])
        row_counts[digest] += 1
    declaration = {
        "anomaly_codes": list(result.anomaly_codes),
        "expected_headers": None if expected_headers is None else list(expected_headers),
        "header_record_count": len(header_values),
        "normalized_output_sha256": result.normalized_output_sha256,
        "presence": result.presence,
        "raw_cell_count": result.cell_count,
        "raw_row_occurrence_count": result.row_count,
        "sequence_row_count": len(result.sequence_rows_by_ordinal),
    }
    pending: list[tuple[str, _ProjectionRecordMetadata, object]] = [
        ("missing_expected" if result.presence == "missing" else "result_set", {}, declaration),
        ("raw_headers", {}, raw_headers),
        ("raw_rows", {}, raw_rows),
    ]
    for header_ordinal, value in enumerate(header_values):
        pending.append(
            (
                "header",
                {
                    ...
                    "header_name": value if type(value) is str else None,
                    "header_ordinal": header_ordinal,
                    "header_value_sha256": header_value_digests[header_ordinal],
                    "header_duplicate_ordinal": header_duplicates[header_ordinal],
                },
                value,
            )
        )
    ...
    for row_ordinal, value in enumerate(row_values):
        row_meta: _ProjectionRecordMetadata = {
            "row_ordinal": row_ordinal,
            "row_value_sha256": row_digests[row_ordinal],
            "row_duplicate_ordinal": row_duplicates[row_ordinal],
        }
        pending.append(("row", row_meta, value))
        if type(value) is not list:
            continue
        for header_ordinal, cell in enumerate(cast("list[object]", value)):
            slot = slots[header_ordinal]
            cell_sha = hashlib.sha256(
                _canonical_json_bytes(cell, maximum=MAX_RESULT_CANONICAL_BYTES)
            ).hexdigest()
            duplicate_key = (header_ordinal, cell_sha)
            duplicate = value_duplicates[duplicate_key]
            value_duplicates[duplicate_key] += 1
            cell_meta: _ProjectionRecordMetadata = {
                **row_meta,
                ...
                "header_duplicate_ordinal": (
                    None
                    if header_ordinal >= len(header_duplicates)
                    else header_duplicates[header_ordinal]
                ),
                "cell_ordinal": cell_ordinal,
                "value_duplicate_ordinal": duplicate,
            }
            pending.append(("cell", cell_meta, cell))
            cell_ordinal += 1
    records: list[DecodedStatsProjectionRecordV1] = []
    for result_record_ordinal, (record_kind, metadata, value) in enumerate(pending):
        records.append(
            _build_projection_record(
                record_ordinal=global_record_start + result_record_ordinal,
                result_record_ordinal=result_record_ordinal,
                record_kind=record_kind,
                result=result,
    ...
    values: dict[str, object] = {
        ...
        "canonical_result_ordinal": result.canonical_ordinal,
        "presence": result.presence,
        "ordered_header_slots_json": slot_json,
        "ordered_header_slots_sha256": hashlib.sha256(slot_json.encode()).hexdigest(),
        "header_record_count": len(header_values),
        "header_slot_count": header_slot_count,
        "raw_row_occurrence_count": result.row_count,
        "sequence_row_count": len(result.sequence_rows_by_ordinal),
        "cell_count": result.cell_count,
        "anomaly_codes_json": _canonical_json_bytes(
            list(result.anomaly_codes), maximum=4096
        ).decode(),
        "anomaly_codes_sha256": hashlib.sha256(
            _canonical_json_bytes(list(result.anomaly_codes), maximum=4096)
        ).hexdigest(),
        "normalized_output_sha256": result.normalized_output_sha256,
        "raw_headers_json": result.canonical_raw_headers_json.decode(),
        "raw_headers_sha256": hashlib.sha256(result.canonical_raw_headers_json).hexdigest(),
        "raw_rows_json": result.canonical_rows_json.decode(),
        "raw_rows_sha256": hashlib.sha256(result.canonical_rows_json).hexdigest(),
        "record_count": len(record_tuple),
        "record_root_sha256": _ordered_projection_root(
            "decoded_stats_projection_result_records_v1",
            tuple(item.record_sha256 for item in record_tuple),
        ),
        "records": record_tuple,
    }
    payload = {
        "schema_version": _PROJECTION_SCHEMA_VERSION,
        "kind": DecodedStatsProjectionResultV1.kind,
        **{
            key: ([item.to_row() for item in record_tuple] if key == "records" else value)
            for key, value in values.items()
        },
    }
    return DecodedStatsProjectionResultV1(
        result_sha256=_projection_sha256(payload), **cast("Any", values)
    ), global_record_start + len(record_tuple)
...
def _json_path(tokens: tuple[str | int, ...]) -> str:
    ...
    for token in tokens:
        path += (
            ...
            else f"[{_canonical_json_bytes(token, maximum=4096).decode()}]"
        )
    if len(path.encode()) > _MAX_PATH_BYTES:
        raise _fail("stats residual JSON path exceeds its bound")
    return path


def _build_residual_nodes(payload: object) -> tuple[DecodedStatsResidualNodeV1, ...]:
    pending: list[
        tuple[object, int | None, tuple[str | int, ...], str | None, int | None, int | None]
    ] = [(payload, None, (), None, None, None)]
    nodes: list[DecodedStatsResidualNodeV1] = []
    while pending:
        value, parent, tokens, object_key, object_key_ordinal, array_ordinal = pending.pop()
        ordinal = len(nodes)
        presence = _projection_presence_kind(value)
        value_kind = _projection_value_kind(value)
        structural = presence == "present" and value_kind in {"array", "object"}
        canonical = (
            None
            if structural
            else _canonical_json_bytes(value, maximum=MAX_RESULT_CANONICAL_BYTES).decode()
        )
        values: dict[str, object] = {
            "node_ordinal": ordinal,
            "parent_node_ordinal": parent,
            "json_path": _json_path(tokens),
            "parent_json_path": None if parent is None else _json_path(tokens[:-1]),
            "depth": len(tokens),
        ...
        identity = {
            "schema_version": _PROJECTION_SCHEMA_VERSION,
            "kind": DecodedStatsResidualNodeV1.kind,
            **values,
        }
        nodes.append(
            DecodedStatsResidualNodeV1(
                node_sha256=_projection_sha256(identity), **cast("Any", values)
            )
        )
        if type(value) is dict:
            children = tuple(cast("dict[str, object]", value).items())
            for child_ordinal in range(len(children) - 1, -1, -1):
                key, child = children[child_ordinal]
                pending.append((child, ordinal, (*tokens, key), key, child_ordinal, None))
        elif type(value) is list:
            children = cast("list[object]", value)
            for child_ordinal in range(len(children) - 1, -1, -1):
                pending.append(
                    (
                        children[child_ordinal],
                        ordinal,
                        (*tokens, child_ordinal),
                        None,
                        None,
                        child_ordinal,
                    )
                )
        if len(nodes) + len(pending) > MAX_JSON_NODES:
            raise _fail("stats residual node budget is exhausted")
    return tuple(nodes)


def decode_stats_projection_response(
    parser_input: bytes,
    *,
    endpoint_id: str,
    endpoint_contract_sha256: str,
    ...
    """Decode one exact response without importing production parser/runtime code."""

    # Pins are admitted by the old observed grammar retained locally in this
    # module, while the exact payload is decoded once more for residual closure.
    observed = decode_observed_lossless_stats_response(
        endpoint_id=endpoint_id,
        endpoint_contract_sha256=endpoint_contract_sha256,
        provider_authority_sha256=provider_authority_sha256,
    ...
    payload = _decode_json_bytes(parser_input, require_object=True)
    contract = _admitted_contract(
        endpoint_id=endpoint_id,
        endpoint_contract_sha256_value=endpoint_contract_sha256,
        provider_authority_sha256=provider_authority_sha256,
    )
    results: list[DecodedStatsProjectionResultV1] = []
    global_record_ordinal = 0
    for result_ordinal, observed_result in enumerate(observed.results):
        result, global_record_ordinal = _build_projection_result(
            result_ordinal=result_ordinal,
            result=observed_result,
            contract=contract,
            global_record_start=global_record_ordinal,
    ...
    nodes = _build_residual_nodes(payload)
    canonical_payload_sha = _projection_sha256(payload)
    anomaly_json = _canonical_json_bytes(list(observed.reason_codes), maximum=4096).decode()
    values: dict[str, object] = {
        ...
        "response_mode": observed.response_mode,
        "provider_result_set_count": observed.provider_result_set_count,
        "expected_result_set_count": observed.expected_result_set_count,
        "anomaly_codes_json": anomaly_json,
        "anomaly_codes_sha256": hashlib.sha256(anomaly_json.encode()).hexdigest(),
        "result_count": len(result_tuple),
        "result_root_sha256": _ordered_projection_root(
            "decoded_stats_projection_results_v1",
            tuple(item.result_sha256 for item in result_tuple),
        ),
        "results": result_tuple,
        "residual_node_count": len(nodes),
        "residual_node_root_sha256": _ordered_projection_root(
            "decoded_stats_projection_residual_nodes_v1", tuple(item.node_sha256 for item in nodes)
        ),
        "residual_nodes": nodes,
        "residual_record_count": len(nodes) + 1,
        "residual_record_root_sha256": _ordered_projection_root(
            "decoded_stats_projection_residual_records_v1",
            (canonical_payload_sha, *(item.node_sha256 for item in nodes)),
        ),
        "canonical_payload_sha256": canonical_payload_sha,
    }
    payload_identity = {
        "schema_version": _PROJECTION_SCHEMA_VERSION,
        "kind": DecodedStatsProjectionResponseV1.kind,
        **{
            key: (
                [item.to_row() for item in result_tuple]
                if key == "results"
                else [item.to_row() for item in nodes]
                if key == "residual_nodes"
                else value
            )
            for key, value in values.items()
        },
    }
    return DecodedStatsProjectionResponseV1(
        response_sha256=_projection_sha256(payload_identity), **cast("Any", values)
    )


def _exact_projection_parser_input(value: object) -> bytes:
    if type(value) is not bytes or not value or len(value) > MAX_PARSER_INPUT_BYTES:
        raise _fail("stats projection parser-input bytes are absent or over bound")
    return value


def _require_exact_projection_source(
    value: DecodedStatsProjectionResponseV1,
    parser_input: bytes,
) -> None:
    if (
        type(value.parser_input_sha256) is not str
        or value.parser_input_sha256 != hashlib.sha256(parser_input).hexdigest()
        or type(value.parser_input_length) is not int
        or value.parser_input_length != len(parser_input)
    ):
        raise _fail("stats projection response differs from its exact parser-input bytes")


def _redecode_exact_projection_source(
    value: DecodedStatsProjectionResponseV1,
    parser_input: bytes,
) -> None:
    expected = decode_stats_projection_response(
        parser_input,
    ...
    if value != expected:
        raise _fail("stats projection response differs from independent parser-input replay")


def validate_decoded_stats_projection_response(
    value: object,
    *,
    parser_input_bytes: bytes,
) -> DecodedStatsProjectionResponseV1:
    """Validate one DTO against the exact built-in parser-input bytes."""

    parser_input = _exact_projection_parser_input(parser_input_bytes)
    if type(value) is not DecodedStatsProjectionResponseV1:
        raise _fail("stats projection response has a foreign exact DTO type")
    exact = value
    _require_exact_projection_source(exact, parser_input)
    candidate = DecodedStatsProjectionResponseV1.from_row(exact.to_row())
    _require_exact_projection_source(candidate, parser_input)
    _redecode_exact_projection_source(candidate, parser_input)
    return candidate


def replay_decoded_stats_projection_response(
    encoded: bytes,
    *,
    parser_input_bytes: bytes,
    expected_response_sha256: str,
    expected_endpoint_contract_sha256: str,
    expected_provider_authority_sha256: str,
) -> DecodedStatsProjectionResponseV1:
    """Replay canonical DTO bytes against exact parser-input bytes and pins."""

    parser_input = _exact_projection_parser_input(parser_input_bytes)
    for value, label in (
        (expected_response_sha256, "expected response"),
        (expected_endpoint_contract_sha256, "expected endpoint contract"),
        (expected_provider_authority_sha256, "expected provider authority"),
    ):
        if type(value) is not str or _SHA256_RE.fullmatch(value) is None:
            raise _fail(f"{label} pin is invalid")
    candidate = DecodedStatsProjectionResponseV1.from_canonical_bytes(encoded)
    if (
        candidate.response_sha256 != expected_response_sha256
        or candidate.endpoint_contract_sha256 != expected_endpoint_contract_sha256
        or candidate.provider_authority_sha256 != expected_provider_authority_sha256
    ):
        raise _fail("stats projection response differs from its external pins")
    _require_exact_projection_source(candidate, parser_input)
    _redecode_exact_projection_source(candidate, parser_input)
    return candidate
