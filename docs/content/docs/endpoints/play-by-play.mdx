...
<div className="grid gap-4 md:grid-cols-3">
  <ScoutCard title="Close the loop with the final line" label="Next stop">
    Pair this page with <a href="/docs/endpoints/box-scores">Box Scores</a> when
    the event stream needs the team and player totals, ratings, and matchup
    summaries keyed by the same <code>game_id</code>.
  </ScoutCard>
  <ScoutCard title="Add off-ball game context" label="Next stop">
    Continue to <a href="/docs/endpoints/other">Other</a> for rotation stints,
    officiating crews, summary packets, and shot-map detail that complement the
    ordered action log.
  </ScoutCard>
  <ScoutCard title="Turn one game into a season question" label="Next stop">
    Use <a href="/docs/endpoints/league-stats">League Stats</a> when a
    possession pattern needs league-wide comparison through standings, lineups,
    play types, or season matchup data.
  </ScoutCard>
</div>
