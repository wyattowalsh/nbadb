...
This table rolls the per-game fact into totals at player-team-season-season-type
grain. A traded player can therefore have multiple team rows in one season, and
regular-season and playoff rows remain distinct. Compare that grain with the
fact's player-game grain; aggregate team rows explicitly when you need one
cross-team player-season total.
