...
<Callout>
If you want to rehearse the SQL structure before you connect to a local `nba.duckdb` file, start with <a href="/docs/playground">SQL Playground</a>. That page runs DuckDB-WASM in the browser with self-contained NBA-flavored sample rows, then this guide shows the same style of reads against the real warehouse tables.
</Callout>
...
| ----------------------------------------- | --- |
| [SQL Playground](/docs/playground) | Browser-only warm-up for DuckDB syntax, CTEs, and NBA-flavored sample queries |
