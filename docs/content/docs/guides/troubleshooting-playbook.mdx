...
therefore API pressure. The scheduler rotates fresh, partial-progress, retry, and
infrastructure lanes while applying an endpoint-family cap when another family is
available. This keeps runners fed and avoids checkpoint barriers between tiny
batches without letting one hot failure family monopolize every runner.
...
| `uv run nbadb upload --verify-remote` | Remote publish stays pending                 | Inspect `logs/kaggle/kaggle-upload-manifest.json`; do not re-upload until the `publication` block proves whether Kaggle exposed the staged publication marker |
