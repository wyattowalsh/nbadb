...
`network_mode=direct`, `vpn_parallelism=1`, and `chunk_profile=micro` before
fan-out. Direct mode skips NordVPN setup, caps the matrix to one active lane at
a time, writes `direct-no-vpn` into lane metadata, and does not require VPN
diagnostic artifacts for failed lanes. Use `network_mode=auto` only after a
smoke proves the direct path is viable and future iterations should prefer VPN
