...
`network_mode=direct`, `direct_parallelism=1`, and `chunk_profile=micro` before
fan-out. Direct mode skips NordVPN setup, uses `direct_parallelism` instead of
VPN fan-out, writes `direct-no-vpn` into lane metadata, and does not require VPN
diagnostic artifacts for failed lanes. Keep direct fan-out low (`1` or `2`)
