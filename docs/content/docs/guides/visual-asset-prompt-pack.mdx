...
<Callout type="warn">
  **Maintainers only:** this prompt pack is for people generating or replacing
  shipped nbadb design assets. It is not part of the normal reader onboarding
  path.
</Callout>
...
<Callout type="info">
  If the model keeps drifting into literal courts, hoops, or poster energy, use
  a hybrid workflow instead: keep the Draw Things plate as the texture or
  lighting source, then compose the final section asset with deterministic
  overlays for court lines, table clusters, route arrows, and scoreboard
  framing. That keeps the result inside the nbadb system even when the raw
  generations are too literal.
</Callout>
