...
| The release job grants `actions: read` and `deployments: write`                    | Durable intent verifies the executor and writes Deployment receipts  |
...
| The job owns `nbadb-kaggle-publish` with `queue: max` and no cancellation          | The ledger verifies the exact cross-workflow publisher mutex         |
...
That job supplies `GH_TOKEN`, `NBADB_KAGGLE_PUBLICATION_SOURCE_SHA`,
`NBADB_KAGGLE_REQUIRE_DEFAULT_HEAD=true`, and, for an approved metadata-only child,
`NBADB_KAGGLE_EXPECTED_DEFAULT_HEAD_SHA`. It also grants `actions: read` and
...
Full, daily, and monthly workflows publish only from the approved default-branch head.
They restore `kaggle-publication-state.json` from the dataset-wide Actions cache before
publication, save it in an `always()` step, and include it in publication artifacts.
That file is secondary recovery evidence. The GitHub Deployment intent plus the
verified `nbadb-kaggle-publish` job mutex is the crash- and cross-host-durable handoff.
Pending publishers wait through `queue: max` instead of replacing one another.
