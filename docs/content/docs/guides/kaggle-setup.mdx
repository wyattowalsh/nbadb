...
`--verify-remote` resolves only `nbadb-publication.json`, not the full dataset. The
resolver must return both the marker and an exact positive dataset version. nbadb
does not infer a version from Kaggle cache-directory names. The resolver validates
the complete marker schema centrally, and persisted publication records must have
valid identity digests, state, transition evidence, and resolved versions before
they can influence another upload.
...
- Use `--verify-remote` for CI or release publishes. It first checks whether Kaggle already exposes the staged bundle identity with exact version evidence, uploads at most once, then polls a small publication marker until its validated resource hashes match or `--remote-timeout` expires. `--remote-poll-interval` controls the polling cadence without repeatedly downloading the full dataset.
- The current upload path validates local metadata/resources, stages only declared files and directories plus the generated publication marker into a temporary upload root, rejects symlinks/path traversal, records staged-tree and remote marker readback identities in `logs/kaggle/kaggle-upload-manifest.json`, and then hands that staged bundle to `kagglehub`.
