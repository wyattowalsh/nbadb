...
- The current upload path validates local metadata/resources, stages only declared files and directories into a temporary upload root, rejects symlinks/path traversal, records a staged-tree fingerprint in `logs/kaggle/kaggle-upload-manifest.json`, and then hands that staged bundle to `kagglehub`.
