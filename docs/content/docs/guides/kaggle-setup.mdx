...
desired contract. Release publication uses the full-extraction assured bundle and
...
| `terminal-assurance-report.json` exists for a full release                         | Full publication binds terminal checkpoint and blocked-lane evidence |
...
uv run nbadb upload --data-dir data/nbadb --message "Describe this publish" --full-publication
...
built; the publisher must consume the exact bytes that passed assurance. It resolves
the exact GitHub artifact ID, verifies the downloaded archive SHA-256 against the
upload result, and revalidates the assured inventory before Kaggle staging.
...
requirements, validates the checkpoint's complete lane/run/coverage inventory, enforces
the exact file-versus-partition-directory resource contract, and compares every table's
ordered schema and row count across DuckDB, SQLite, CSV, and Parquet. Each temporary file
is removed before the next one, bounding readback
...
- The metadata preview generated without `--data-dir` declares 510 resources: two database files plus CSV and Parquet entries for all 254 runtime transform outputs. A concrete complete export also includes all 416 canonical staging surfaces, for 1,342 resources. A fully assured complete publication declares 1,344 after adding the assured artifact manifest and terminal assurance report.
...
- Use `--verify-remote` when any update needs exact Kaggle version and full resource-digest readback. Use `--full-publication` for a terminal full extraction; it implies remote verification and additionally revalidates `assured-artifact-manifest.json` plus the matching terminal assurance report. Both modes upload at most once, then poll until the exact-version API inventory and every streamed file digest match or `--remote-timeout` expires. Defaults are 3,600 seconds and a 15-second poll interval.
