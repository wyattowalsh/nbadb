...
- Use `--verify-remote` for CI or release publishes. It first checks whether Kaggle already exposes the staged bundle identity, uploads at most once, then polls a small publication marker until its validated resource hashes match or `--remote-timeout` expires. `--remote-poll-interval` controls the polling cadence without repeatedly downloading the full dataset.
