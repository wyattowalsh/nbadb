...
<StatGrid columns={4}>
  <StatPill label="Python" value="3.12+" note="required for the current package and source workflow" />
  <StatPill label="Install routes" value="2" note="PyPI for quick use, source checkout for contributors" />
  <StatPill label="Default exports" value="4" note="sqlite, duckdb, csv, and parquet" />
  <StatPill label="Daily lookback" value="7 days" note="default recent-game window for nbadb daily" />
</StatGrid>
...
<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
  <ScoutCard title="Quick package install" label="Entry surface">
    Jump to <a href="#install-from-pypi">Install from PyPI</a> if you just want the CLI on your machine fast.
  </ScoutCard>
  <ScoutCard title="Contributor source setup" label="Entry surface">
    Use <a href="#install-from-source">Install from source</a> if you will edit code, docs, or generated artifacts.
  </ScoutCard>
  <ScoutCard title="Env and defaults" label="Entry surface">
    Go straight to <a href="#configuration-defaults">Configuration defaults</a> for the `NBADB_` knobs that matter first.
  </ScoutCard>
  <ScoutCard title="Sanity-check the setup" label="Entry surface">
    Skip to <a href="#first-possession-checklist">First possession checklist</a> if install already succeeded and you need to validate it.
  </ScoutCard>
</div>
...
<InsightCard title="Useful download detail">
  After `nbadb download`, nbadb will seed `nba.duckdb` from `nba.sqlite` if the download produced SQLite but not DuckDB. That gives you a usable DuckDB file without forcing a fresh historical rebuild.
</InsightCard>
