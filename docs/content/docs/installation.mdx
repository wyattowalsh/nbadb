...
### 4. Run the standard recurring refresh

```bash
nbadb daily
```

That command refreshes the current/open season, applies the configured recent-game
window, persists applicable result routes and active live-game sources, then rebuilds
the complete stable output universe. It uses the current local/Kaggle dataset; no
private repository, capture store, or generation pointer is required. See
[Daily Updates](/docs/guides/daily-updates) for exact scope, failure, and hosted
