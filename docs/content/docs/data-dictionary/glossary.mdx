...
<Callout type="info">
  Percentages and rates are usually stored as decimals (`0.0-1.0`) unless the
  source metric is traditionally expressed another way. Always pair the metric
  with the table grain: the same stat name means something different on a
  player-game row than on a season aggregate. Use this page for meaning; use the
  generated inventory pages for exact table-by-table field presence.
</Callout>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="Shooting efficiency and shot value" label="Quick route">
    Start with <a href="#shooting-efficiency">Shooting Efficiency</a> for{" "}
    <code>TS%</code>, <code>eFG%</code>, <code>FG%</code>, <code>FG3%</code>,{" "}
    <code>FT%</code>, and free-throw-rate style fields.
  </ScoutCard>
  <ScoutCard title="Impact and usage metrics" label="Quick route">
    Go to <a href="#player-impact-metrics">Player Impact Metrics</a> for{" "}
    <code>PIE</code>, <code>USG%</code>, <code>AST%</code>, <code>AST/TO</code>,
    and <code>AST Ratio</code>.
  </ScoutCard>
  <ScoutCard
    title="Rebounding, turnovers, and possession control"
    label="Quick route"
  >
    Use{" "}
    <a href="#rebounding-and-turnover-metrics">
      Rebounding and turnover metrics
    </a>{" "}
    for <code>OREB%</code>, <code>DREB%</code>, <code>REB%</code>, and
    turnover-rate context.
  </ScoutCard>
  <ScoutCard
    title="Pace, ratings, and team quality signals"
    label="Quick route"
  >
    Open <a href="#pace-and-efficiency-ratings">Pace and efficiency ratings</a>{" "}
    for pace, offensive rating, defensive rating, and net rating.
  </ScoutCard>
  <ScoutCard title="Four Factors and Synergy shorthand" label="Quick route">
    ...
    <a href="#synergy-and-possession-type-shorthand">
      Synergy and possession-type shorthand
    </a>{" "}
    when the question is about team-winning drivers or possession
    classifications.
  </ScoutCard>
  <ScoutCard
    title="Box score, hustle, tracking, and win probability"
    label="Quick route"
  >
    Jump to <a href="#box-score-basics">Box score basics</a>,{" "}
    <a href="#hustle-and-tracking-shorthand">Hustle and tracking shorthand</a>,
    or <a href="#win-probability-shorthand">Win probability shorthand</a> for
    the fastest operational definitions.
  </ScoutCard>
</div>
...
<DataColumns>
  <InsightCard title="Check the grain">
    A stat on a player-game row, a team-game row, and a season aggregate can
    share the same name but answer different questions. Grain first,
    interpretation second.
  </InsightCard>
  <InsightCard title="Check the stat family">
    Decide whether the metric is about shooting efficiency, possession load,
    pace/rating context, tracking, or event counts. That narrows the reading
    lane fast.
  </InsightCard>
...
<DataColumns>
  <InsightCard title="How to use this page">
    Treat the formulas here as reading aids for warehouse metrics, not as
    guarantees that every table exposes every intermediate term. The same named
    metric may appear at different grains with slightly different surrounding
    context columns.
  </InsightCard>
  <InsightCard title="Where to go after the definition">
    Once you understand the term, switch to{" "}
    <a href="/docs/data-dictionary/field-reference">Field Reference</a> for
    naming patterns or to the generated{" "}
    <a href="/docs/data-dictionary/star">Star</a> inventory when you need to
    confirm exactly which public tables expose the field.
  </InsightCard>
</DataColumns>
