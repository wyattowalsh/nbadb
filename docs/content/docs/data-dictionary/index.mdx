...
<Callout type="info">
  Use the curated pages for interpretation and navigation. Use the generated
  tier pages when you need the exact field inventory for a specific
  schema-backed layer. If you are deciding <em>what a field means</em>, start
  curated. If you are checking <em>whether a field exists</em>, go generated.
</Callout>
...
<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
  <ScoutCard title="Glossary" label="Meaning first">
    Start with <a href="/docs/data-dictionary/glossary">Glossary</a> when the
    problem is a metric, acronym, formula, or box-score shorthand such as{" "}
    <code>TS%</code>, <code>PIE</code>, or <code>PPP</code>.
  </ScoutCard>
  <ScoutCard title="Field Reference" label="Pattern first">
    Start with{" "}
    <a href="/docs/data-dictionary/field-reference">Field Reference</a> when the
    question is really about naming habits: keys, suffixes, home/visitor labels,
    rating names, or row discriminators.
  </ScoutCard>
  <ScoutCard title="Tier inventories" label="Contract first">
    Start with <a href="/docs/data-dictionary/raw">Raw</a>,{" "}
    <a href="/docs/data-dictionary/staging">Staging</a>, or{" "}
    <a href="/docs/data-dictionary/star">Star</a> when you need to verify
    whether a field exists on a specific tier right now.
  </ScoutCard>
  <ScoutCard title="Ownership trail" label="Where it came from">
    Jump to <a href="/docs/lineage">Lineage</a> or{" "}
    <a href="/docs/schema">Schema Reference</a> when the real blocker is not
    meaning but ownership, join path, or transform lineage.
  </ScoutCard>
</div>
...
<DataColumns>
  <InsightCard title="Curated pages explain how to read">
    They translate field families, abbreviations, and naming conventions into
    analyst-facing guidance.
  </InsightCard>
  <InsightCard title="Generated pages explain what exists">
    They are command-owned snapshots of the current field inventory, not
    narrative documentation.
  </InsightCard>
</DataColumns>

## Contract inventory vs reviewed lineage

<Callout type="warn">
  An inventory match is not reviewed lineage. The contract compiler preserves
  candidates so maintainers can review them, but same-name, source-metadata,
  dependency, SQL-passthrough, and normalized-name matches remain non-green
  until they receive an explicit cross-layer disposition.
</Callout>

| Inventory slice               | Count authority                    | Current admission state                                                                |
| ----------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------- |
| Provider-field occurrences    | Fresh field-fate contract artifact | Every occurrence needs an explicit reviewed model and metric disposition               |
| Unique provider identities    | Fresh field-fate contract artifact | Route-local repeats remain separate evidence rather than being silently deduplicated   |
| Public numeric columns        | Fresh metric-contract artifact     | Candidates stay quarantined from semantic consumers until reviewed                     |
| Structural numeric nonmetrics | Fresh metric-contract artifact     | Explicit key, relationship, or uniqueness evidence excludes them from metric admission |
| Unreviewed numeric candidates | Fresh metric-contract artifact     | They still need formula, grain, unit, aggregation, eligibility, and use-case review    |
| Computed numeric SQL aliases  | Fresh metric-contract artifact     | An exact normalized expression alone does not review its metric semantics              |

Use the assurance artifacts for occurrence-level blockers and current digests. Do
not promote candidate evidence into docs recommendations, chat routes, notebook
metrics, or other semantic consumers until its registry contract admits that use.
...
- [Schema Reference](/docs/schema) — curated family guides for dimensions, facts, bridges, aggregations, and analytics views
- [Dimensions](/docs/schema/dimensions) — identity, history, calendar, and lookup context tables
- [Facts & Bridges](/docs/schema/facts) — measurement tables and many-to-many connectors
- [Analytics Views](/docs/schema/analytics-views) — 14 pre-joined convenience surfaces
- [Relationships](/docs/schema/relationships) — join playbook with SQL examples
- [Lineage](/docs/lineage) — transform dependency chain and data flow
