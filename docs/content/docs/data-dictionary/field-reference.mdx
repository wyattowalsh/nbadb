...
<Callout type="info">
  Need the exact field list for a specific schema-backed table? Jump to the
  generated [Raw](/docs/data-dictionary/raw),
  [Staging](/docs/data-dictionary/staging), or
  [Star](/docs/data-dictionary/star) inventory. Use this page to decode how
  those fields behave.
</Callout>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="Find the join anchors first" label="Quick route">
    Jump to <a href="#identity-and-grain-columns">Identity and grain columns</a>{" "}
    when you need to decide whether a row is keyed by player, team, game,
    season, date, or lineup/group context.
  </ScoutCard>
  <ScoutCard title="Decode who the row is talking about" label="Quick route">
    Use <a href="#role-and-context-columns">Role and context columns</a> for
    home vs visitor, offense vs defense, participant-slot fields, and shot-zone
    labels.
  </ScoutCard>
  <ScoutCard title="Read the stat name by sight" label="Quick route">
    Open <a href="#metric-naming-quick-read">Metric naming quick read</a> when
    the question is really about suffixes like <code>{"_pct"}</code>,{" "}
    <code>{"_rank"}</code>, <code>{"total_"}</code>, or rolling windows.
  </ScoutCard>
  <ScoutCard title="Explain the duplicate-looking rows" label="Quick route">
    Go to <a href="#discriminator-columns">Discriminator columns</a> when
    repeated keys might actually be different slices, summaries, or tracking
    cuts rather than bad duplication.
  </ScoutCard>
</div>
...
<DataColumns>
  <InsightCard title="Fast reading sequence">
    Read dense generated tables in this order: <strong>grain</strong> first,
    then <strong>discriminator</strong>, then <strong>role/context</strong>, and
    only after that the measures. That order prevents most bad joins and most
    stat-name misreads.
  </InsightCard>
  <InsightCard title="Where exactness lives">
    This page explains patterns that repeat across the warehouse. The
    authoritative exhaustive field lists still live on the generated{" "}
    <a href="/docs/data-dictionary/raw">Raw</a>,{" "}
...
<Callout>
  If a table exposes both <code>&lt;entity&gt;_id</code> and{" "}
  <code>&lt;entity&gt;_sk</code>, read them as different jobs, not duplicate
  columns. The natural key answers <em>who</em>; the surrogate/history key
  answers <em>which versioned row</em>.
</Callout>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="Natural key vs history key" label="Trap to avoid">
    Do not treat <code>&lt;entity&gt;_id</code> and{" "}
    <code>&lt;entity&gt;_sk</code> as interchangeable. The natural key answers{" "}
    <em>who</em>; the surrogate/history key answers <em>which row version</em>.
  </ScoutCard>
  <ScoutCard title="Home/visitor labels are not noise" label="Trap to avoid">
    On game-centered surfaces, <code>home_team_id</code> and{" "}
    <code>visitor_team_id</code> define the matchup context. They are part of
    the row's story, not decorative lookup columns.
  </ScoutCard>
  <ScoutCard
    title="Totals and averages should not be mixed casually"
    label="Trap to avoid"
  >
    <code>total_pts</code> and <code>avg_pts</code> can sit near each other on
    season surfaces but answer different questions. Check the grain before
    comparing them to player-game rows.
  </ScoutCard>
  <ScoutCard title="Repeated IDs are often intentional" label="Trap to avoid">
    If repeated keys differ on <code>detail_type</code>,{" "}
    <code>summary_type</code>, <code>tracking_type</code>, or{" "}
    <code>split_type</code>, you are probably looking at parallel analytical
    cuts rather than a broken dedupe.
  </ScoutCard>
</div>

<Callout>
  When the table is still too dense, switch to the schema page first:
  [Dimensions](/docs/schema/dimensions), [Facts](/docs/schema/facts), [Derived
  Aggregations](/docs/schema/derived), or [Analytics
  Views](/docs/schema/analytics-views). Use the curated page to understand the
  table family, then come back to the generated field inventory for exact
  columns.
</Callout>
