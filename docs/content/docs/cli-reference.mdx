...
- `--full-publication` makes every declared conformed and silver/gold game, roster, team, box-score, play-by-play, shot, standings, draft, aggregate, and analytics anchor mandatory and enforces declared row-preserving silver-to-gold cardinality pairs
...
| `nbadb scan`            | Scan database for missing data, gaps, and quality issues        | —                   | `--data-dir/-d`, `--category/-c`, `--table/-t`, `--severity/-s`, `--fail-on/-F`, `--report-path/-r`, `--output-format/-f`, `--ci`, `--full-publication`, `--checkpoint-report`, `--checkpoint-manifest`, `--checkpoint-dir`, `--checkpoint-chain-id`, `--checkpoint-source-sha` |
...
| `nbadb upload`                      | Upload the dataset directory to Kaggle                                                 | —               | `--data-dir/-d`, `--message/-m` (default: `"Automated update"`), `--verify-remote`, `--full-publication`, `--remote-timeout` (default: `3600`), `--remote-poll-interval` (default: `15`), `--publication-ledger` (default: `local`; `local` or `github-deployment`), `--require-durable-intent` |
