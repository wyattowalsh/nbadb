...
| `nbadb upload`                      | Publish the data directory to Kaggle                                | `--verify-remote` proves exact remote identity; `--full-publication` also requires terminal assurance         |
...
- With `--verify-remote`, the marker resolver must return an exact positive Kaggle version. The client then paginates that version's complete API file list and streams every file through SHA-256 readback. Only a marker-specific HTTP 404 can bootstrap one upload after metadata-API version resolution; other baseline failures stop before upload. Any unresolved dataset upload blocks every later bundle until exact evidence resolves it. CLI retries depend on retained `logs/kaggle/kaggle-publication-state.json`; publishing workflows persist that file in their shared default-branch cache.
...
| `nbadb upload`                      | Upload the dataset directory to Kaggle                                                 | —               | `--data-dir/-d`, `--message/-m` (default: `"Automated update"`), `--verify-remote`, `--full-publication`, `--remote-timeout` (default: `3600`), `--remote-poll-interval` (default: `15`) |
