...
<Callout type="warn">
**Deprecated commands:** `nbadb full` has been deprecated in favor of `nbadb backfill run`, which provides finer-grained control over gap detection and targeted re-extraction. `nbadb run-quality` has been deprecated in favor of `nbadb scan`.
</Callout>

<Callout type="info">
One subtle but important behavior: `daily` and `monthly` finish by rebuilding downstream tables in <strong>replace</strong> mode. They are not row-level upsert commands against the public star surface.
</Callout>
