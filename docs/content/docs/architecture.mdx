...
| `nbadb backfill run` | Targeted gap fill                | Extracts specific endpoints/seasons with filters, supports `--force`, `--extract-only`, and `--dry-run` |

<Callout type="warn">
  **Deprecated commands:** `nbadb full` has been deprecated in favor of `nbadb
  backfill run`, which provides finer-grained control over gap detection and
  targeted re-extraction. `nbadb run-quality` has been deprecated in favor of
  `nbadb scan`.
</Callout>

<Callout type="info">
  One subtle but important behavior: `daily` and `monthly` finish by rebuilding
  downstream tables in <strong>replace</strong> mode. They are not row-level
  upsert commands against the public star surface.
</Callout>
...
| Facts             |   128 | `fact_`      | Grain-specific measurements and events                   |
| Bridges           |     5 | `bridge_`    | Join helpers for many-to-many relationships              |
| Aggregates        |    19 | `agg_`       | Reusable rollups for repeated analysis                   |
| Analytics outputs |    12 | `analytics_` | Analysis-ready convenience surfaces                      |
