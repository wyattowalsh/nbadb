---
title: Dimensions
description: Playbook for the current dimension outputs that define identity, calendar, venue, and lookup context
---
...
<StatGrid columns={3}>
  <StatPill
    ...
    value={siteInventory.tableFamilyCounts.dimensions}
...
<Callout>
  <strong>SCD Type 2 alert:</strong> `dim_player` and `dim_team_history`
  preserve change over time. Use their business keys for ordinary joins, and
  reach for surrogate/history fields only when you need time-aware snapshots.
</Callout>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="Anchor hubs" label="Jump to section">
    Go to <a href="#anchor-hubs">Anchor hubs</a> for the tables that carry most
    analyst joins: player, team, game, and season/date context.
  </ScoutCard>
  <ScoutCard title="Supporting context" label="Jump to section">
    Jump to <a href="#supporting-context">Supporting context</a> when the
    question is about people, venues, franchise history, or lookup vocabularies
    rather than the primary join hubs.
  </ScoutCard>
  <ScoutCard title="Join anchors" label="Jump to section">
    Use <a href="#join-anchors-and-watchouts">Join anchors and watchouts</a>{" "}
    when you need the safest key to bring a dimension into a query.
  </ScoutCard>
  <ScoutCard title="Exact contracts" label="Generated boundary">
    Switch to the generated{" "}
    <a href="/docs/schema/star-reference">Star Reference</a> once you know the
    dimension family and need exact schema-backed columns or constraints.
  </ScoutCard>
</div>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="dim_player" label="Primary hub">
    Start here for current player identity, position, jersey, and team context.
    It is the default player join for most fact families unless you explicitly
    need a broader lookup-only roster list.
  </ScoutCard>
  <ScoutCard title="dim_team" label="Primary hub">
    The safest current-team join for names, abbreviations, conference/division
    context, and arena linkage. Reach for `dim_team_extended` only when the
    baseline team surface is too thin.
  </ScoutCard>
  <ScoutCard title="dim_game" label="Primary hub">
    The game anchor for both sides of the matchup. It carries `game_id` plus
    `home_team_id` and `visitor_team_id`, so you can add both clubs without
    inventing a separate matchup table.
  </ScoutCard>
  <ScoutCard title="Season + date dimensions" label="Primary hub">
    Use `dim_season`, `dim_season_phase`, `dim_season_week`, and `dim_date` when
    the question is temporal first: reporting windows, weekday splits, regular
    season vs playoffs, or NBA-specific calendar framing.
  </ScoutCard>
</div>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="People and school context" label="Content module">
    `dim_all_players`, `dim_coach`, `dim_official`, and `dim_college` support
    broader person lookup, staffing, officiating, and background workflows that
    do not belong on every fact row.
  </ScoutCard>
  <ScoutCard
    title="Franchise, venue, and history context"
    label="Content module"
  >
    `dim_team_history`, `dim_defunct_team`, and `dim_arena` cover branding
    changes, relocated franchises, retired identities, and venue metadata that
    would otherwise clutter the core team and game dimensions.
  </ScoutCard>
  <ScoutCard title="Controlled vocabularies" label="Content module">
    `dim_play_event_type` and `dim_shot_zone` decode repeated codes and
    geography into readable labels. They are small tables with outsized value
    because they keep event and shot facts interpretable.
  </ScoutCard>
  <ScoutCard title="History-aware joins" label="Content module">
    When time-aware history matters, use the SCD2 fields on `dim_player` or the
    historical rows in `dim_team_history`. For ordinary analyst joins, stick to
    the business keys and current rows.
  </ScoutCard>
</div>
...
<Callout type="info">
  Use this page as the scouting summary. When you need the exact schema-backed
  contract for a specific dimension, jump to the generated{" "}
