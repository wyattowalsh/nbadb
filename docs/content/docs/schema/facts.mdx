---
title: Facts & Bridges
description: Scouting report for the current fact and bridge tables in the nbadb warehouse
---
...
<StatGrid columns={3}>
  <StatPill
    label="Fact outputs"
    value={siteInventory.tableFamilyCounts.facts}
    note="game, season, event, dashboard, and specialty fact tables"
  />
  <StatPill
    label="Bridge tables"
...
| Possession, lineup, and model-input facts   | `fact_statistical_possession`, `fact_lineup_stint`, `fact_rapm_design`                                                                              | Possession event, exact-five stint, or unfitted design row | You need conservative possession boundaries, lineup responses, or reproducible RAPM inputs  |
| Season, profile, on/off, and dashboards     | `fact_player_profile`, `fact_player_career`, `fact_player_splits`, `fact_team_splits`, `fact_team_on_off_overall`, `fact_league_player_on_details`  | Player/team season, provider group, or dashboard split     | You need descriptive season or on/off context without implying causal player impact         |
...
<div className="grid gap-4 md:grid-cols-3">
  ...
  <ScoutCard title="bridge_live_box_score_official" label="Bridge table">
    Preserves the officials attached to each live box-score snapshot, keyed by
    snapshot time, game, and official person ID without conflating the snapshot
    feed with the historical game-official bridge.
  </ScoutCard>
</div>

| Bridge                           | Join pattern                                            | Why it exists                                                                                             |
| -------------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `bridge_game_official`           | `game_id` + `official_id`                               | Attaches historical officiating crews without repeating referee columns on every game-level row           |
| `bridge_live_box_score_official` | `snapshot_at` + `game_id` + `person_id`                 | Keeps live-snapshot officials distinct from historical game-official observations                         |
...
| Bridges are not optional cleanup                           | All six bridges, including the live-snapshot official bridge, exist specifically to prevent repeated columns and awkward multi-slot joins         |
