...
<Callout type="info">
This page is the scouting report, not the full scorebook. Use it to choose the right fact family, then jump to the generated <a href="/docs/schema/star-reference">Star Reference</a> when you need exact schema-backed columns and constraints.
</Callout>
...
<div className="grid gap-4 md:grid-cols-2">
  <ScoutCard title="Choose a fact family" label="Jump to section">
    Start with <a href="#fact-families">Fact families</a> when you need the closest grain match for a player-game, team-game, event, dashboard, or league-style question.
  </ScoutCard>
  <ScoutCard title="Read by question" label="Jump to section">
    Use <a href="#read-the-box-score-by-question">Read the box score by question</a> when you already have a query shape in mind and need the quickest first table.
  </ScoutCard>
  <ScoutCard title="Handle many-to-many joins" label="Jump to section">
    Jump to <a href="#current-bridge-tables">Current bridge tables</a> when officials or play participants would otherwise force repeated columns or duplicate joins.
  </ScoutCard>
  <ScoutCard title="Escalate or de-escalate grain" label="Generated boundary">
    Move up to <a href="/docs/schema/derived">Derived Aggregations</a> or <a href="/docs/schema/analytics-views">Analytics Views</a> for reusable shortcuts, or down to <a href="/docs/schema/star-reference">Star Reference</a> when the exact contract matters.
  </ScoutCard>
</div>
...
<div className="grid gap-4 md:grid-cols-3">
  ...
  <ScoutCard title="bridge_game_team" label="Bridge table">
    Links each game to its two participating teams with a `side` discriminator (`home`/`away`) and win/loss outcome. Useful when you need both sides of a game as separate rows.
  </ScoutCard>
  <ScoutCard title="bridge_lineup_player" label="Bridge table">
    Explodes opaque `group_id` strings from lineup stats into individual player rows, so you can query "find all lineups containing player X" without parsing IDs yourself.
  </ScoutCard>
