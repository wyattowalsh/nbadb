...
| Aggregate | `fg_pct` | Re-computed from player-team-season-type totals for accuracy |

**Key transformation**: In `agg_player_season`, `fg_pct` is recomputed within each
player-team-season-season-type row as `SUM(fgm) / SUM(fga)` rather than averaging
per-game percentages, which would be statistically incorrect.
...
**Key transformation**: `avg_ts_pct` is computed within each player-team-season-
season-type row as a simple average of per-game values in the current
implementation. For a totals-based result, recompute `PTS / (2 * (FGA + 0.44 *
FTA))`.
...
**Key transformation**: Per-game ratings flow directly to the fact table.
`agg_player_season` computes player-team-season-season-type averages, while
`agg_team_pace_and_efficiency` computes team-level pace/efficiency summaries.
