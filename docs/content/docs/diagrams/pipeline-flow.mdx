...
carry exact, attested DuckDB state artifacts into the next iteration. Workload-bound
...
only an explicit attested pointer is carried forward. Split children clear parent
...
flowchart TD
    ...
    BundleGate -->|yes| ExtractMatrix["Extract lane matrix"]
...
requires a full route, a changed exit IP, a strict `TeamYears` result-set response,
and bounded `common_all_players` plus `league_game_log` canaries through the installed
`nbadb`/`nba_api` stack. The player canary requires positive player/team membership.
...
Preflight failures join the effective chain quarantine before fan-out and are persisted
into each child manifest. The seed requests
...
lane database digest, lane fields, exact coverage hash, and every manifest season/type.
...
manifests never extend it. The dispatcher searches paginated
...
exactly one newly visible child run. Direct-mode workflow concurrency uses chain and
iteration, so the parent cannot hold the child's slot. VPN/auto chains instead share
