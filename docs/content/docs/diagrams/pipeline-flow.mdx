...
extraction in smaller temporal lanes and durable checkpoint aggregation. The
checkpoint is the recovery boundary: each redispatch only needs the previous
checkpoint plus newly completed lane databases, and terminal merge consumes the
checkpoint before export. Incomplete lanes can also carry durable DuckDB state
artifacts into the next iteration when the cache is missing or unusable.

```mermaid
flowchart TD
    Plan["Plan lanes\nchunk_profile"] --> ExtractMatrix["Extract lane matrix"]
    ...
    LaneControl --> Checkpoint["Checkpoint aggregate\nnba.duckdb"]
    Checkpoint --> Decision{"Active lanes remain?"}
    Decision -->|yes| Dispatch["Dispatch next iteration\nmanifest handoff"]
    Dispatch --> Plan
    Decision -->|no| Merge["Terminal merge\ncheckpoint first"]
    Merge --> TransformOnly["Transform-only load"]
    TransformOnly --> ExportAll["Export + publish"]
```
