...
source-run recovery are mutually exclusive. A recovery launch preserves the original
chain and pinned source while naming a distinct completed `resume_source_run_id`.
...
digest-mismatch failure enabled. A bounded name-based fallback remains only for
...
posts `workflow_dispatch` with `return_run_details=true`, validates the returned child
run ID and URLs, and re-reads that exact run to verify title, event, head SHA, and
source before acknowledgement. The committed next-manifest uses a run/attempt-unique
non-overwriting artifact; its ID and digest are forwarded to the child for REST
ID/name/digest/size/run/source verification and exact-ID download. Run/name-only
handoff is a bounded legacy path only when both receipt fields are absent. It still
requires exact source-owner/workflow identity, stable unique inventory, a direct
...
manifest, and enters the exact non-cancelling `nbadb-kaggle-publish` FIFO queue. It
requires `actions: read`, `deployments: write`, `GH_TOKEN`, the publication source
...
it. Metadata is generated for all 254 runtime transform outputs. The publisher uses
`--full-publication` to require both assurance files and matching terminal provenance;
generic `--verify-remote` remains available for nonterminal updates.
