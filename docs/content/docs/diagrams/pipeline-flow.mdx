...
handoff is a bounded legacy path when both receipt fields are absent. If the parent
exits first, its trap attempts to cancel that exact returned child ID. Direct-mode workflow concurrency uses chain and iteration,
