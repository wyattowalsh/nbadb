...
def get_player_info_helper(player, proxies):
    while True:
        try:
            df = CommonPlayerInfo(player_id=player, proxy=np.random.choice(proxies), timeout=3).get_data_frames()[0]
            df.columns = df.columns.to_series().apply(lambda x: x.lower())
            return df
        except RequestException:
            continue
...
def get_teams_details_helper(team, proxies):
    ...
    while True:
        try:
            res_dfs = TeamDetails(team_id=team, proxy=np.random.choice(proxies), timeout=3).get_data_frames()
...
def get_box_score_summaries(game_ids, proxies, save_to_db=False, conn=None):
    ...
    line_score = pd.concat([d['line_score'] for d in dfs if d['line_score'] is not None]).reset_index(drop=True)
    if save_to_db:
